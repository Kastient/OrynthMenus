...
Кнопка формы поддерживает заголовок и не более двух строк `lore`. Если `text`/`display_name` не указан, первая строка `lore` становится заголовком. `image.type` принимает `path` для встроенной/ресурсной текстуры Bedrock и `url` для изображения по HTTPS. Также работают короткие поля `image-path` и `image-url`.
...
Те же параметры доступны в игре: Shift + клик по предмету → **Атрибуты и подсказки**. Переключатели записывают канонические имена в `item_flags`, поэтому ручная правка YAML и редактор дают одинаковый результат.
